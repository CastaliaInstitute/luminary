...
5. Grades the same geometry for York solar state; sunset remains off-screen while warming the horizon and reflection. A moon is drawn only when measured altitude/azimuth places it inside the camera view, before cloud compositing so clouds can occlude it.
